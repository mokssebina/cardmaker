    ...
    return (
        <div className='w-full flex flex-col mt-6'>
            ...
            <Textarea name={name} as={Fragment}>
                {({ focus, hover }) => (
                    <textarea placeholder={placeholder} className={clsx('h-52 p-1 border border-gray-500 rounded-lg', focus && 'bg-blue-100', hover && 'shadow')} value={value} onChange={onChange}></textarea>
                )}
            </Textarea>
        </div>
    )
